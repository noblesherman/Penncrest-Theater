...
- File: `src/pages/admin/ScannerLive.tsx`
...
export default function AdminScannerLivePage() {
  ...
  const enqueueOffline = (scannedValue: string, clientScanId: string) => {
    const next = enqueueOfflineScannerItem({
      performanceId,
    ...
    setNotice({ kind: 'error', text: `Offline — queued ${next.length} scan${next.length === 1 ? '' : 's'}.` });
    setFlash('warn');
  };
  ...
  const submitScan = async (scannedValue: string) => {
    ...
    try {
      if (!navigator.onLine) {
        enqueueOffline(scannedValue, clientScanId);
        return null;
      }

      const result = await adminFetch<ScanResponse>('/api/admin/check-in/scan', {
        method: 'POST',
        body: JSON.stringify({ performanceId, sessionToken: scannerSession!.sessionToken, scannedValue, clientScanId })
      });
  ...
  const syncOfflineQueue = async () => {
    ...
    const queued = readOfflineScannerQueue();
    if (queued.length === 0) {
      setOfflineQueue([]);
      return;
    }
    ...
    try {
      const storedSessions = readStoredScannerSessions();
      ...
      for (let index = 0; index < queued.length; index += 1) {
        ...
        try {
          ...
          if (message.includes('session is not active') || message.includes('unauthorized') || message.includes('forbidden')) {
            remaining.push(item, ...queued.slice(index + 1));
            blockedMessage = 'Queue sync paused — start a scanner session for the affected performance.';
            break;
          }
        }
      }

      writeOfflineScannerQueue(remaining);
  ...
  const startSession = async () => {
    ...
    try {
      ...
      persistScannerSessionForPerformance(session.performanceId, session);
  ...
  useEffect(() => {
    ...
    adminFetch<PerformanceRow[]>('/api/admin/performances?scope=active&kind=all')
      .then((rows) => {
        ...
        if (active.length > 0) {
          const initialPerformanceId = active[0].id;
          setPerformanceId(initialPerformanceId);
          const storedSession = readStoredScannerSessions()[initialPerformanceId];
          if (storedSession) {
  ...
  useEffect(() => {
    ...
    setScannerSession((current) => {
      ...
      return readStoredScannerSessions()[performanceId] || null;
    });
    stopCamera();
  }, [performanceId]);
  ...
  }, [performanceId, scannerSession, isSyncingQueue]);
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden bg-black" style={{ fontFamily: "var(--font-sans)" }}>
      ...
      <header className="absolute inset-x-0 top-0 z-20">
        ...
        {(offlineQueue.length > 0 || isSyncingQueue) && (
          <div className="mx-4 mb-2 rounded-xl border border-amber-500/40 bg-amber-950/85 px-4 py-2 text-xs text-amber-200 backdrop-blur-sm">
            {isSyncingQueue
        ...
        {showSessionPanel && (
          <div className="mx-4 mb-2 rounded-2xl border border-white/15 bg-zinc-950/95 p-4 shadow-xl backdrop-blur-md">
            ...
            {!sessionReady ? (
              ...
              <div className="flex flex-col gap-3 rounded-xl border border-emerald-800 bg-emerald-950 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
        ...
        <div className="flex gap-2">
          ...
          {offlineQueue.length > 0 && (
            <button
              ...
              className="rounded-2xl border border-amber-500/30 bg-amber-950/80 px-4 py-4 text-sm font-semibold text-amber-200 backdrop-blur-sm disabled:opacity-60"
            >
              {isSyncingQueue ? '...' : offlineQueue.length}
            </button>
          )}
        </div>
