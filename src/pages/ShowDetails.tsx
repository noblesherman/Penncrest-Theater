...
- File: `src/pages/ShowDetails.tsx`
...
import { SITE_ADDRESS, SITE_FAVICON_PATH, SITE_NAME } from '../lib/siteMeta';
  ...
  useEffect(() => {
    fetch(apiUrl(`/api/shows/${id}`))
      .then(async (res) => {
        ...
        const safePosterUrl = typeof data.posterUrl === 'string' ? data.posterUrl : '';
  ...
  const structuredData = [
    {
      ...
      image: showPosterUrl ? [showPosterUrl] : [],
      ...
      <div className="min-h-screen bg-white pb-14 sm:pb-20">
        ...
        <div className="relative h-[52vh] min-h-[420px] overflow-hidden bg-stone-900 sm:h-[60vh] sm:min-h-[500px]">
          ...
          <div className="absolute inset-0 opacity-40">
            {showPosterUrl ? (
              <img src={showPosterUrl} alt={show.title} className="w-full h-full object-cover blur-sm" />
            ) : null}
          </div>
                      ...
                      <div key={member.id} className="flex-none w-40 md:w-48 snap-start group">
                        <div className="relative aspect-[3/4] bg-stone-200 rounded-xl overflow-hidden mb-3 shadow-md group-hover:shadow-xl transition-all duration-300 group-hover:-translate-y-1">
                          {member.photoUrl ? (
                            ...
                            <div className="flex h-full flex-col items-center justify-center bg-gradient-to-b from-stone-200 to-stone-300 text-stone-500">
                              <img src={SITE_FAVICON_PATH} alt="" className="mb-2 h-12 w-12 object-contain opacity-60" />
                              <div className="text-xs uppercase tracking-[0.14em] font-semibold">No Photo</div>
                            </div>
                          )}
                        </div>
