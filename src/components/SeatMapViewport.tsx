...
- File: `src/components/SeatMapViewport.tsx`
...
export function SeatMapViewport<T extends SeatMapSeat>({
  ...
  const hasRenderableSeats = renderedSeats.length > 0;
  ...
  const fitMapToViewport = useCallback(
    (animationTime = 220) => {
      ...
      const centeredY = (wrapperHeight - mapContent.height * clampedScale) / 2;
  ...
  useEffect(() => {
    if (loading || !hasRenderableSeats || hasInitialFitRef.current) return;
  ...
  }, [fitMapToViewport, hasRenderableSeats, loading]);
  ...
  return (
    ...
    >
      ...
      {!hasRenderableSeats && !loading ? (
        <div className="absolute inset-0 flex items-center justify-center p-6">
          {emptyState ?? (
            <div className="w-full max-w-md rounded-2xl border border-stone-200 bg-white/95 p-5 text-center shadow-sm backdrop-blur-sm">
              <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-stone-500">Seating</p>
              <p className="mt-2 text-base font-semibold text-stone-900">{emptyText}</p>
            </div>
          )}
        </div>
      ) : (
        ...
        >
          <TransformComponent
            wrapperClass={`!h-full !w-full ${isPanning ? 'cursor-grabbing' : 'cursor-grab'}`}
