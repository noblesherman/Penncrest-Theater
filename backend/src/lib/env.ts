...
const EnvSchema = z.object({
  ...
  ENABLE_IN_PROCESS_PAYMENT_LINE_WORKER: booleanFromEnv(!isProductionEnv),
  ...
  ENABLE_IN_PROCESS_TICKET_EMAIL_OUTBOX_WORKER: booleanFromEnv(true),
  TICKET_EMAIL_OUTBOX_WORKER_SWEEP_INTERVAL_SECONDS: z.coerce.number().int().min(1).max(120).default(5),
  TICKET_EMAIL_OUTBOX_PROCESSING_TIMEOUT_SECONDS: z.coerce.number().int().min(10).max(600).default(90),
  TICKET_EMAIL_OUTBOX_BATCH_SIZE: z.coerce.number().int().min(1).max(200).default(25),
  TICKET_EMAIL_OUTBOX_MAX_ATTEMPTS: z.coerce.number().int().min(1).max(20).default(6),
  TICKET_EMAIL_OUTBOX_RETRY_BASE_SECONDS: z.coerce.number().int().min(1).max(600).default(15),
  ...
  EMAIL_PROVIDER_SIGNING_SECRET: z.string().trim().min(1).optional(),

  STAFF_CLAIM_CODE: z.string().optional(),
  STAFF_REDEEM_CODE_TTL_MINUTES: z.coerce.number().int().min(5).max(60 * 24 * 30).default(60 * 24 * 7),
  STAFF_COMP_RESERVE_IP_WINDOW_SECONDS: z.coerce.number().int().min(30).max(60 * 60).default(10 * 60),
  STAFF_COMP_RESERVE_IP_MAX_ATTEMPTS: z.coerce.number().int().min(1).max(200).default(20),
  STAFF_COMP_RESERVE_EMAIL_WINDOW_SECONDS: z.coerce.number().int().min(30).max(60 * 60).default(10 * 60),
  STAFF_COMP_RESERVE_EMAIL_MAX_ATTEMPTS: z.coerce.number().int().min(1).max(100).default(6),
  STAFF_COMP_RESERVE_CODE_WINDOW_SECONDS: z.coerce.number().int().min(30).max(60 * 60).default(10 * 60),
  STAFF_COMP_RESERVE_CODE_MAX_ATTEMPTS: z.coerce.number().int().min(1).max(200).default(12),
  STAFF_COMP_RESERVE_FAILURE_WINDOW_SECONDS: z.coerce.number().int().min(30).max(60 * 60).default(10 * 60),
  STAFF_COMP_RESERVE_FAILURE_LOCK_THRESHOLD: z.coerce.number().int().min(1).max(50).default(5),
  STAFF_COMP_RESERVE_LOCKOUT_MINUTES: z.coerce.number().int().min(1).max(12 * 60).default(15),
