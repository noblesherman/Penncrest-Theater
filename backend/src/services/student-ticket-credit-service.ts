...
- File: `backend/src/services/student-ticket-credit-service.ts`
...
export async function reserveStudentCreditForOrder(params: {
  ...
  await prisma.$transaction(async (tx) => {
    await reserveStudentCreditForOrderTx(tx, params);
  });
}

export async function reserveStudentCreditForOrderTx(
  tx: Prisma.TransactionClient,
  params: {
    orderId: string;
    studentTicketCreditId: string;
...
export async function releasePendingStudentCreditForOrder(orderId: string): Promise<number> {
  return prisma.$transaction(async (tx) => releasePendingStudentCreditForOrderTx(tx, orderId));
}

export async function releasePendingStudentCreditForOrderTx(
  tx: Prisma.TransactionClient,
  orderId: string
): Promise<number> {
  const order = await tx.order.findUnique({
