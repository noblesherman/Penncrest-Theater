...
- File: `backend/src/services/stripe-checkout-finalization 2.ts`
...
async function finalizePaidOrderTx(
  ...
  const refreshedOrderSeats =
    missingOrderSeatRows.length > 0
      ? await tx.orderSeat.findMany({
